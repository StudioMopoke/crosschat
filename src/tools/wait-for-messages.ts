...
export function registerWaitForMessages(server: McpServer, messageStore: MessageStore): void {
  server.tool(
    ...
    'Wait for the next message in your current room. Blocks until a message arrives or timeout.',
    {
      ...
      broadcastCooldownMs: z.number().optional().describe('Delay in ms before returning broadcast messages (not direct @mentions). Used to stagger responses across agents so earlier responders can be seen by later ones. Set this to a random value (0-8000) seeded at init time.'),
    },
